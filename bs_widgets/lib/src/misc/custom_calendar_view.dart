...
class CustomCalendarView extends StatefulWidget {
  ...
  ///
  /// It will display a grid of days for the [selectedDates]'s month, or, if that
  /// is null, the [currentDate]'s month. The day indicated by [selectedDates] will
  ...
  ///
  /// The [selectedDates], if provided, must be between [firstDate] and [lastDate]
  ...
  ///
  /// If [selectableDayPredicate] and [selectedDates] are both non-null,
  /// [selectableDayPredicate] must return `true` for the [selectedDates].
  CustomCalendarView({
    super.key,
  ...
  }) : selectedDates = selectedDates?.map(DateUtils.dateOnly).toList(),
       firstDate = DateTime(
         (firstDate ?? currentDate ?? DateTime.now()).year,
         (firstDate ?? currentDate ?? DateTime.now()).month,
         1,
       ),
    ...
    assert(
      !lastDate.isBefore(this.firstDate),
      'lastDate $lastDate must be on or after firstDate ${this.firstDate}.',
    );
    assert(
      !(this.selectedDates?.any(
            (element) => element.isAfter(this.currentDate),
          ) ??
          false),
      'selectedDates must not contain a date after current date: ${this.currentDate}.',
    );
    // assert(
  ...
  /// change the [key] to create a new instance of the [CustomCalendarView], and
  /// provide that widget the new [selectedDates]. This will reset the widget's
  /// interactive state.
...
class _CustomCalendarViewState extends State<CustomCalendarView> {
  ...
  void initState() {
    ...
    final currentDisplayedDate = DateUtils.isSameMonth(widget.currentDate, widget.lastDate)
        ? widget.lastDate
        : widget.currentDate;
  ...
  Widget _buildPicker() => switch (_mode) {
    DatePickerMode.day => _MonthPicker(
      key: _monthPickerKey,
  ...
  Widget build(BuildContext context) {
    ...
    final scaledMaxDayPickerHeight = textScaleFactor > 1.3
...
class _DatePickerModeToggleButtonState
    extends State<_DatePickerModeToggleButton> /*with SingleTickerProviderStateMixin*/ {
  ...
  Widget build(BuildContext context) {
    ...
    final controlColor = colorScheme.onSurface.withAlphaFraction(0.60);
                ...
                child: SizedBox(
                  ...
                  child: StaticData.targetPlatform.isApple
                      ? CupertinoButton(
                          ...
                          child: _toggleButtonChild(textTheme, controlColor),
                        )
                      : InkWell(
  ...
  Padding _toggleButtonChild(TextTheme textTheme, Color controlColor) => Padding(
      ...
      children: <Widget>[
        Flexible(
          child: Text(
            ...
            style: textTheme.bodyLarge?.copyWith(color: controlColor),
          ),
        ),
        RotatedBox(
          // RotationTransition(
          // turns: _controller,
          quarterTurns: widget.mode == DatePickerMode.year ? 2 : 0,
          child: Icon(AppStyle.icons.arrowDown, color: controlColor),
        ),
      ],
    ),
  );
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void _handleGridFocusChange(bool focused) {
    setState(() {
      if (focused && _focusedDay == null) {
        if (DateUtils.isSameMonth(widget.selectedDates?.first, _currentMonth)) {
          _focusedDay = widget.selectedDates?.first;
  ...
  @override
  Widget build(BuildContext context) {
    final controlColor = Theme.of(
      context,
    ).colorScheme.onSurface.withAlphaFraction(0.60);
  ...
  Widget build(BuildContext context) {
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        // || widget.currentDate.isBefore(dayToBuild)
        ;
        ...
        dayItems.add(
          _Day(
            ...
            canBeSelected: dayToBuild.isBefore(widget.currentDate),
...
class _DayState extends State<_Day> {
  ...
  Widget build(BuildContext context) {
    ...
    final dayShape = resolve<OutlinedBorder?>(
        ...
        child: Center(
          child: Text(
            ...
            style: dayStyle?.apply(
              // color: widget.canBeSelected ? dayForegroundColor : Theme.of(context).disabledColor,
              color: widget.isToday && widget.isSelectedDay ? Colors.white : dayForegroundColor,
            ),
          ),
        ),
      ),
    ...
    if (widget.isDisabled) {
      ...
      dayWidget = InkResponse(
        ...
        onTap: !widget.canBeSelected ? null : () => widget.onChanged(widget.day),
  ...
  Color _getTopColor() => Colors.white.withAlpha(50);
...
class _DayPickerGridDelegate extends SliverGridDelegate {
  ...
  SliverGridLayout getLayout(SliverConstraints constraints) {
    final textScaleFactor =
        ...
        _fontSizeToScale;
    final scaledRowHeight = textScaleFactor > 1.3
...
/// [CustomCalendarView], or [showDatePicker] which create full date pickers.
///
/// See also:
///
///  * [CustomCalendarView], which provides a Material Design date picker
///    interface.
///
...
class _YearPickerState extends State<YearPicker> {
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    ...
    final decorationHeight = 36.0 * textScaleFactor;
    final decorationWidth = 72.0 * textScaleFactor;
    ...
    final overlayColor = WidgetStateProperty.resolveWith<Color?>(
      (Set<WidgetState> states) => effectiveValue(
        (DatePickerThemeData? theme) => theme?.yearOverlayColor?.resolve(states),
      ),
    );
    ...
    final itemStyle = (datePickerTheme.yearStyle ?? defaults.yearStyle)?.apply(
      color: textColor,
    );
...
class _YearPickerGridDelegate extends SliverGridDelegate {
  ...
  SliverGridLayout getLayout(SliverConstraints constraints) {
    final textScaleFactor =
        ...
        _fontSizeToScale;
    final scaledYearPickerColumnCount = textScaleFactor > 1.65
    ...
    final scaledYearPickerRowHeight = textScaleFactor > 1
