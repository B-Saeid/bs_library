...
import 'dismissable_clipper.dart';
...
const double _kActionThreshold = 0.25;
...
///
/// {@tool dartpad}
/// This sample shows how you can use the [BsDismissible] widget to
...
class BsDismissible extends StatefulWidget {
  /// Creates a widget that can be dismissed.
  ///
  /// The [key] argument is required because [BsDismissible]s are commonly used in
  ...
  /// The offset threshold the item has to be dragged in order to
  /// call [actionCallback]
  ///
  ...
  /// Called when the dismissible widget has been dragged
  /// passed the value within [actionThresholds].
  ///
  /// Note: [actionCallback] will be called exactly once per
  /// swipe gesture.
  final VoidCallback? actionCallback;

  @override
  State<BsDismissible> createState() => _BsDismissibleState();
}

class _BsDismissibleState extends State<BsDismissible>
  ...
  late final AnimationController _moveController = AnimationController(
    ...
    reverseDuration: widget.movementReverseDuration,
    vsync: this,
  );
  ...
  double get _actionThreshold => widget.actionThresholds[_dismissDirection] ?? _kActionThreshold;
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    if (!_dragUnderway || _moveController.isAnimating) return;
  ...
  void _handleDismissUpdateValueChanged() {
    if (widget.actionCallback != null) {
      /// If
      ///   - the user has left his finger
      ///   - after he dragged past the action threshold
      ///   - and the actionCallback has not been scheduled to be called.
      ///
      /// do schedule the actionCallback to be called.
      if (!_dragUnderway && _moveController.value > _actionThreshold && !_shouldCallAction) {
        _shouldCallAction = true;
      }
    }
  ...
  void _handleDragEnd(DragEndDetails details) {
    ...
    final flingVelocity = _directionIsXAxis
    ...
    switch (_describeFlingGesture(details.velocity)) {
      ...
      case _FlingGestureKind.none:
        if (!_moveController.isDismissed) {
          ...
          /// Copied from [dismissThresholds] doc above:
          ///
          /// Setting a threshold of 1.0 (or greater) prevents a >>drag<<
          /// - not drag .. prevents dismissing - in the given
          /// [DismissDirection] even if it would be allowed by the [direction]
          /// property.
          ///
          /// This is what we will use to build [Swipeable.action]
          ///
          /// ==========
          ///
          /// This `if` is entered when drag ends after [_dragUnderway] is set false:
          ///
          /// First it checks [_moveController.value] - which ranges from 0.0 to 1.0 -
          /// if it is greater than [_dismissThreshold]
          ///
          /// if yes -> _moveController.forward(); until animation is completed
          /// to trigger [_handleDismissStatusChanged] to trigger [_handleMoveCompleted] ...
          ///
          /// You can now see the flow more clearly.
          ///
          /// The Else path is interesting to us, as we need to call [actionCallback]
          /// once in the whole process and exactly when the all animations end.
  ...
  Future<void> _handleDismissStatusChanged(AnimationStatus status) async {
    if (status.isCompleted && !_dragUnderway) await _handleMoveCompleted();
    if (status.isDismissed && _shouldCallAction) widget.actionCallback!();
  ...
  Future<void> _handleMoveCompleted() async {
    ...
    final result = await _confirmStartResizeAnimation();
    if (mounted) {
      if (result) {
