...
// _TODO(abarth): These constants probably belong somewhere more general.
...
// _TODO(LongCatIsLooong): https://github.com/flutter/flutter/issues/39272. Use
...
//   final scaledFontSize = MediaQuery.maybeTextScalerOf(context)?.scale(defaultFontSize);
...
//     final backgroundColor = CupertinoDynamicColor.resolve(_kDialogColor, context);
...
// `_SlideTarget`s.
//
// _TODO(dkwingsmt): It should recompute hit testing when the app is updated,
...
class CustomCupertinoActionSheet extends StatefulWidget {
  ...
  ///
  /// If this is not a list of [CustomCupertinoActionSheetAction]
  /// you may consider setting [customActions] to true.
  ///
  final List<Widget>? actions;

  /// A flag to opt-out of the familiar behaviour
  /// of cupertino sliding actions.
  final bool customActions;

  /// A scroll controller that can be used to control the scrolling of the
  /// [message] in the action sheet.
  ///
  /// Defaults to null, which means the [CustomCupertinoActionSheet] will create a
  /// scroll controller internally.
  ...
  ///
  /// Defaults to null, which means the [CustomCupertinoActionSheet] will create an
  /// action scroll controller internally.
  final ScrollController? actionScrollController;
  ...
  /// actions.
  ///
  /// This must be a [CustomCupertinoActionSheetAction] widget.
  final Widget? cancelButton;

  @override
  State<CustomCupertinoActionSheet> createState() => _CustomCupertinoActionSheetState();
}

class _CustomCupertinoActionSheetState extends State<CustomCupertinoActionSheet> {
  int? _pressedIndex;
  ...
  Widget _buildCancelButton() {
    ...
    final cancelPadding = (widget.actions != null || widget.message != null || widget.title != null)
...
/// The content of a typical action button in a [CustomCupertinoActionSheet].
///
/// This widget draws the content of a button, i.e. the text, while the
/// background of the button is drawn by [CustomCupertinoActionSheet].
///
/// See also:
///
...
class CustomCupertinoActionSheetAction extends StatefulWidget {
  ...
  @override
  State<CustomCupertinoActionSheetAction> createState() => _CustomCupertinoActionSheetActionState();
}

class _CustomCupertinoActionSheetActionState extends State<CustomCupertinoActionSheetAction>
  ...
  Widget build(BuildContext context) {
    ...
    final contextBodySize = MediaQuery.textScalerOf(context).scale(higLargeBodySize);
...
//   @override
//   Widget build(BuildContext context) {
//     final dialogColor = CupertinoDynamicColor.resolve(_kDialogColor, context);
//     final dialogPressedColor = CupertinoDynamicColor.resolve(_kDialogPressedColor, context);
//     final dividerColor = CupertinoDynamicColor.resolve(CupertinoColors.separator, context);
//
//     final column = <Widget>[];
//     for (var actionIndex = 0; actionIndex < actions.length; actionIndex += 1) {
...
class _AlertDialogButtonBackgroundState extends State<_AlertDialogButtonBackground>
  ...
  @override
  Widget build(BuildContext context) {
    final backgroundColor = widget.pressed ? widget.pressedColor : widget.idleColor;
    return MetaData(
      metaData: this,
...
//
//     if (!enabled) {
//       style = style.copyWith(color: style.color!.withAlpha(255 ~/ 2));
//     }
...
//     // _TODO(mattcarroll): The following logic is not entirely correct. It is also
